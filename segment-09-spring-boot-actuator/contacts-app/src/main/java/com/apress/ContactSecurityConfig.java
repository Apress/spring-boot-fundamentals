...
public class ContactSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth.userDetailsService( email -> {
            ...
            Contact contact = this.contactRepository.findById(email).orElseThrow(() -> new RuntimeException("User not found!"));
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
            ...
            .requestMatchers(EndpointRequest.toAnyEndpoint()).hasAnyRole("ACTUATOR")
